			       ...
			       u8 **out_edid)
{
	...
	if (out_edid)
		*out_edid = edid;
	if (!edid) {
		pr_info("radeonfb: I2C (port %d) ... not found\n", conn);
		return MT_NONE;
	}
	if (edid[0x14] & 0x80) {
		...
		if (rinfo->is_mobility /*&& conn == ddc_dvi*/ &&
			...
			pr_info("radeonfb: I2C (port %d) ... found LVDS panel\n", conn);
			return MT_LCD;
		} else {
			pr_info("radeonfb: I2C (port %d) ... found TMDS panel\n", conn);
			return MT_DFP;
		}
	}
	pr_info("radeonfb: I2C (port %d) ... found CRT display\n", conn);
	return MT_CRT;
}
